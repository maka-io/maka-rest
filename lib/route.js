        ...
        allowedMethods.forEach((method) => {
            ...
            return JsonRoutes.add(method, fullPath, async (req, res) => {
                ...
                if (responseData.body && (responseData.statusCode || responseData.headers)) {
                    return await self._respond(res, responseData.body, responseData.statusCode, responseData.headers);
                } else {
                    return await self._respond(res, responseData);
                }
            });
        });

        return rejectedMethods.forEach(method => JsonRoutes.add(method, fullPath, (req, res) => {
            const responseData = {status: 'error', message: 'API endpoint does not exist'};
