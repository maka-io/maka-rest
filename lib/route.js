...
class Route {
  ...
  _authenticate(endpointContext) {
    // Get auth info
    const { token, error } = this.api._config.auth.getAuthToken.call(endpointContext);
    ...
    if (token) {
      const userSelector = {};
      userSelector[this.api._config.auth.token] = token;
      userObj = Meteor.users.findOne(userSelector);
    }

    if (error !== null) { return { success: false, data: error }; }

    // Attach the user and their ID to the context if the authentication was successful
    // TODO: Why?
    if (userObj) {
      //endpointContext.user = userObj.user;
      //endpointContext.userId = userObj._id;
      return { success: true , data: { token, ...userObj } };
    } else {
