    ...
    */
    ...
    _validateUser(user, password) {
        ...
        if (!user.services.password.srp) {
            ...
            console.log(resultOfInvocation);
      ...
      */
        this.addRoute('login', {authRequired: false}, {
            post() {
                ...
                try {
                    const isValidUser = self._validate({ username: user.username, email: user.email, password });
                    if (isValidUser) {
                    ...
                    return{
                        statusCode: e.error,
                        body: { status: 'error', message: new Meteor.Error('Error attempting login', e) }
                    };
                }
